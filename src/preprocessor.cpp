...
Preprocessor::Preprocessor(const std::vector<std::string>& includePaths)
    : includePaths(includePaths), inAsmBlock(false) {}
...
std::optional<std::string> Preprocessor::preprocess(const std::string& filename) {
    ...
    while (std::getline(stream, line)) {
        ...
        if (inAsmBlock) {
            if (handleAsmEnd(line)) {
                asmBlocks.push_back(currentAsmBlock);
                output << "INLINE_ASM\n"; // Replace with inline assembly token
                currentAsmBlock = AsmBlock(); // Reset current block
                continue;
            }
            currentAsmBlock.lines.push_back(line);
            ...
            handleDefine(line);
            output << "\n"; // Remove the #define line from output
        } else if (line.find("#undef") == 0) {
            handleUndef(line);
            output << "\n"; // Remove the #undef line from output
        } else if (line.find("#header") == 0) {
            handleHeader(line, output);
        } else if (line.find("#asmstart") == 0) {
            handleAsmStart(line);
        } else if (line.find("#asmend") == 0) { // Ensure asmend is handled
            handleAsmEnd(line);
        } else {
...
bool Preprocessor::handleInclude(const std::string& line, const std::string& currentDir, std::ostringstream& output) {
    ...
    while (std::getline(stream, fileLine)) {
        ...
        if (inHeaderBlock && fileLine.find("#endheader") == 0) {
            headerContent << fileLine << "\n";
            inHeaderBlock = false;
            break;
        }
...
bool Preprocessor::handleAsmStart(const std::string& line) {
    inAsmBlock = true;
    currentAsmBlock.lines.clear();
    return true;
}

bool Preprocessor::handleAsmEnd(const std::string& line) {
    inAsmBlock = false;
    return true;
}
...
const std::vector<AsmBlock>& Preprocessor::getAsmBlocks() const {
    return asmBlocks;
}
